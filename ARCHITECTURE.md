...
owns Dotos round-trip witnesses, it explicitly enables
`signal-frame/dotos-text` through its own default `dotos-text` feature
...
| No stringly-typed dispatch (`match s.as_str()`) for closed-set states. | All target / backend / health / readiness / reason fields are typed closed enums. `SystemTarget` carries a hand-written Dotos codec (the variant head IS structural) but does not parse free text. |
...
`SystemTarget` is the exception: it has a hand-written Dotos codec so
...
The manifest binds Signal Frame, Persona, and Dotos to exact immutable
producer revisions.
